	...
	if (tr.fraction < 1.0) {
		...
		if (traceEnt->client != NULL) {
			...
			if ( (traceEnt->client->ps.pm_type == PM_DEAD) && (traceEnt->client->sess.sessionTeam == ent->client->sess.sessionTeam)) {
				...
				GLOBALSTATS(ent, GLOBAL_REVIVES, ent->client->pers.revives);
...
// jpw
			...
			GLOBALSTATS(ent, GLOBAL_MGREPAIRED, ent->client->pers.mgRepair);
		}
		else
			traceEnt->health += 3;
	}
		...
		if ( traceEnt->methodOfDeath == MOD_DYNAMITE ) {
			...
			if ( traceEnt->s.teamNum >= 4 ) {
				...
				GLOBALSTATS(ent, GLOBAL_DYNOPLANTED, ent->client->pers.dynoPlanted);
				...
				if ( traceEnt->health >= 248 ) {
					...
					for ( i=0 ; i<num ; i++ ) {
						...
						if (!strcmp(hit->classname,"trigger_objective_info")) {
							...
							GLOBALSTATS(ent, GLOBAL_DYNODISARMED, ent->client->pers.dynoDisarmed);
						}
					}
				}
	// jpw
...
// L0 - air strike block 
		...
		if ((tr.entityNum > 0) && (tr.entityNum < MAX_CLIENTS) && (g_entities[tr.entityNum].inuse) && g_easyASBlock.integer) {			
			...
			GLOBALSTATS(&g_entities[tr.entityNum], GLOBAL_ASBLOCKED, g_entities[tr.entityNum].client->pers.ASblocked);
		} 
// end
	...
	GLOBALSTATS(ent->parent, GLOBAL_ASTHROWN, ent->parent->client->pers.ASthrown);
	...
	if (level.time - ent->client->ps.classWeaponTime > g_LTChargeTime.integer) {
		...
		if ( ent->client->sess.sessionTeam == TEAM_RED && level.axisArty > level.time)
		{
			CP(va("chat \"HQ: ^3All available planes are already en-route. Plane available in %i seconds.\n\"",
				((level.axisArty - level.time) / 1000 ) + 1));
		...
		{
			CP(va("chat \"HQ: ^3All available planes are already en-route. Plane available in %i seconds.\n\"",
				((level.alliedArty - level.time) / 1000 ) + 1));
			//CPS(ent, "sound/multiplayer/radar_loop02.wav");
			return;
		} // End
		...
		GLOBALSTATS(ent, GLOBAL_ASCALLED, ent->client->pers.AScalled);
...
//		bomb->s.weapon	= WP_SMOKE_GRENADE;
        // TTimo ambiguous else
			...
			bomb->methodOfDeath			= MOD_ARTILLERY;	// L0 - was MOD_AIRSTRIKE
			bomb->splashMethodOfDeath	= MOD_ARTILLERY;	// L0 - was MOD_AIRSTRIKE
			bomb->clipmask = MASK_MISSILESHOT;
			bomb->s.pos.trType = TR_STATIONARY; // was TR_GRAVITY,  might wanna go back to this and drop from height
			bomb->s.pos.trTime = level.time;		// move a bit on the very first frame
...
void Bullet_Fire (gentity_t *ent, float spread, int damage ) {
	...
	GLOBALSTATS(ent, GLOBAL_SHOTSFIRED, ent->client->pers.acc_shots);
}
...
//----(SA)	end
	...
	if ( traceEnt->takedamage && traceEnt->client && !(traceEnt->flags & FL_DEFENSE_GUARD) ) {
		...
		if( LogAccuracyHit( traceEnt, attacker ) ) {
			...
			GLOBALSTATS(attacker, GLOBAL_SHOTSHIT, attacker->client->pers.acc_hits);
		}
...
// L0 - AS limit
			...
			if ( ent->client->sess.sessionTeam == TEAM_RED && level.axisBomber > level.time)
			{
				CP(va("chat \"HQ: ^3Planes will be available in %d seconds!\n\"",
					((level.axisBomber - level.time) / 1000 ) + 1));
				//CPS(ent, "sound/multiplayer/radar_loop02.wav");
				return;
			}
			...
			{
				CP(va("chat \"HQ: ^3Planes will be available in %d seconds!\n\"",
					((level.alliedBomber - level.time) / 1000 ) + 1));
				//CPS(ent, "sound/multiplayer/radar_loop02.wav");
				return;
			} 
// End
