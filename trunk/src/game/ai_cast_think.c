...
//----(SA)	end
	...
	if (cs->actionFlags & CASTACTION_WALK)
		...
		movechar = 127;
	if (bi->actionflags & ACTION_MOVEFORWARD) ucmd->forwardmove = movechar;
	if (!(cs->aiFlags & AIFL_WALKFORWARD)) {	// only do other movements if we are allowed to
		if (bi->actionflags & ACTION_MOVEBACK) ucmd->forwardmove = -movechar;
		if (bi->actionflags & ACTION_MOVELEFT) ucmd->rightmove = -movechar;
		if (bi->actionflags & ACTION_MOVERIGHT) ucmd->rightmove = movechar;
	}
	//jump/moveup
	if (bi->actionflags & ACTION_JUMP) ucmd->upmove = 127;	// JUMP always takes preference over ducking
	if (bi->actionflags & ACTION_MOVEDOWN) ucmd->upmove = -127;	// JUMP always takes preference over ducking
	if (bi->actionflags & ACTION_MOVEUP) ucmd->upmove = 127;	// JUMP always takes preference over ducking
	//
		...
		if (pm->touchents[i] < aicast_maxclients )
		{
			...
			if (	(ocs->bs) &&
				...
				if (ocs->leaderNum >= 0) {
					...
					ogoal.entitynum = ocs->leaderNum;
					if (ocs->bs && AICast_GetAvoid(ocs, &ogoal, ocs->obstructingPos, qfalse, cs->entityNum))
					{	// give them time to move somewhere else
						ocs->obstructingTime = level.time + 1000;
					}
				} else {
					if (ocs->bs && AICast_GetAvoid(ocs, NULL, ocs->obstructingPos, qfalse, cs->entityNum))
					{	// give them time to move somewhere else
						ocs->obstructingTime = level.time + 1000;
					}
				}
