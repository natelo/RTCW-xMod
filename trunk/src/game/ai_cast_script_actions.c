...
#define	SCRIPT_REACHGOAL_DIST	8
  ...
  // TTimo gcc: suggest parentheses around assignment used as truth value
	...
	if (Distance( cs->bs->origin, ent->r.currentOrigin ) < SCRIPT_REACHGOAL_DIST)
	{	// we made it
		return qtrue;
	}
...
#define	SCRIPT_REACHCAST_DIST	64
	...
	if (Distance( cs->bs->origin, ent->r.currentOrigin ) < SCRIPT_REACHCAST_DIST)
	{	// we made it
		return qtrue;
	}
...
//----(SA)	some weapons always go together (and they share a clip, so this is okay)
		if(weapon == WP_GARAND)			COM_BitSet( g_entities[cs->entityNum].client->ps.weapons, WP_SNOOPERSCOPE);
		if(weapon == WP_SNOOPERSCOPE)	COM_BitSet( g_entities[cs->entityNum].client->ps.weapons, WP_GARAND);
		if(weapon == WP_FG42)			COM_BitSet( g_entities[cs->entityNum].client->ps.weapons, WP_FG42SCOPE);
		if(weapon == WP_BAR)			COM_BitSet( g_entities[cs->entityNum].client->ps.weapons, WP_BAR2);
		if(weapon == WP_DYNAMITE)		COM_BitSet( g_entities[cs->entityNum].client->ps.weapons, WP_DYNAMITE2);
//----(SA)	end
