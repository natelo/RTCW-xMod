...
int BotCTFTeam(bot_state_t *bs) {
	...
	if (Q_stricmp(p, CTF_SKIN_REDTEAM) == 0) return CTF_TEAM_RED;
	if (Q_stricmp(p, CTF_SKIN_BLUETEAM) == 0) return CTF_TEAM_BLUE;
...
//			bs->inventory[INVENTORY_SLUGS] > 5) return 95;
	...
	if (bs->inventory[INVENTORY_FLAMETHROWER] > 0 &&
			bs->inventory[INVENTORY_FUEL] > 50) return 90;
	...
	if (bs->inventory[INVENTORY_SP5] > 0 &&
			bs->inventory[INVENTORY_SP5AMMO] > 40) return 85;
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		...
		if (curenemy < 0 && (healthdecrease || EntityIsShooting(&entinfo))) fov = 360;
