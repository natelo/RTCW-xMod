...
// JPW NERVE
		...
		if (!other->droppedObj)
		{
			...
			other->client->pers.stats.objSteals++;
		}
...
// L0 - Different prints for tossing obj..
			...
			else
			{
				...
				//APS("sound/multiplayer/axis/g-objective_taken.wav");
				//matchInfo(MT_ME, va("Axis have tossed %s!", ent->message));
			} 
// End
			if ( gm )
...
// L0 - Different prints for tossing obj..
			...
			else
			{
				...
				//APS("sound/multiplayer/allies/a-objective_taken.wav");
				//matchInfo(MT_ME, va("Allies have tossed %s!", ent->message));
			}
// End
			if ( gm )
	...
	if (g_gametype.integer != GT_WOLF_CP) {
		if (g_flagRetake.integer == 0) {			
			...
			if (FlagBalance() != 0) {
				...
				{	
					if (!g_balanceFlagCanClaim.integer)
					{
